...
export const getAvailableAssetsToRequestQuery = async (employeeId:number):Promise<Asset[]> => {
    const query = `
        ...
        WHERE status = 'unassigned'
          AND id NOT IN (
            SELECT asset_id
            FROM request_asset
            WHERE employee_id = $1 AND status IN ($2)
        )
    `;
    try {
        const result = await pool.query<Asset>(query,[employeeId,'pending']);
        return result.rows;
    } catch (err) {
        console.log('error getting available assets ' + err);
        throw err;
